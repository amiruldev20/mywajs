...
class Message extends Base {
    ...
    _patch(data) {
         ...
         * @type {string}
         */
        this.deviceType = typeof data.id.id === 'string' && data.id.id.length > 21 ? 'android' : typeof data.id.id === 'string' && data.id.id.substring(0, 2) === 'MYWA' ? 'ios' : 'web';
        /**
         ...
         * @type {boolean}
         */
        this.hasQuotedMsg = (data.quotedMsg && data.quotedStanzaID && data.quotedParticipant) ? true : false;
    ...
    async downloadMedia() {
        ...
        const result = await this.client.playPage.evaluate(async (msg) => {
            try {
